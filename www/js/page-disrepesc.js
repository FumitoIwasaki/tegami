app.controller('SubController10',['$scope','$rootScope','INQUIRE1','Authenticator', function($scope, $rootScope, INQUIRE1, Authenticator) {

// -----------------------------------------------------------------------------
//  広域災害状況報告：ESC：初期画面での報告時の処理

  $scope.disescDialog1 = function(tmpStr) {
    var result_doc = '';
    var latitude4 = '';
    var longitude4 = '';
    var seido = '';
...
//          ons.notification.alert({ title: "確認", message: "登録しました" });
  ...
  $scope.disescDialog2 = function(tmpStr) {
//  function disescDialog2(tmpStr) {
    var obj = { disresult: tmpStr };
    localStorage.setItem("disRESULT", JSON.stringify(obj));

    app.navi.pushPage('disrepescdetail.html', { animation : 'slide' } );
  }
