...
app.controller('SubController_Main_Search',['$scope','$rootScope','INQUIRE','FJ01sql','Authenticator', function($scope, $rootScope, INQUIRE,FJ01sql, Authenticator) {
...
// 検索ボタンをクリックしたときの処理
// *****************************************************************************
  $scope.Main_Search_go = function() {
      
    //検索項目の変数をクリア（グローバル変数のためクリアしておく）
    main_pronect_no="";
          
    //プロジェクトNo
    main_pronect_no=document.Main_Search.tProject_No.value; 
    
    //工番
    main_tkoban=document.Main_Search.tTkoban.value;
    alert("[koban]" + main_tkoban);
    //データ確認
    //console.log(main_pronect_no); 
     
    app.navi.pushPage('Main_Search_list.html');
    $(document).on('pageinit','#Main_Search_list', function() {
      var groupno = "";
